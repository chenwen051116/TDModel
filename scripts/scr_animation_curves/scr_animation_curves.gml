...
function exponential_anim_curve(spd0, spd1, init_abruptness) {
    var exponent_coef = ln(spd1 + 1 - spd0);
    return make_lambda(function(cap, val) {
        if (val < 0 || val > 1) throw("val should be within the range [0, 1]");
        return exp(cap.exponent_coef * power(val, cap.init_abruptness)) + cap.spd0 - 1;
    }, { exponent_coef: exponent_coef, init_abruptness: init_abruptness, spd0: spd0 });
}
